...
export function DiscoveryRadar({
  ...
  const normalizeDropPaths = (payload: unknown): string[] => {
    ...
    if (Array.isArray(payloadWithPaths.paths)) {
      return payloadWithPaths.paths.filter(
        (value): value is string => typeof value === "string",
      );
    }
    ...
    >
      ...
      <div className="pointer-events-none absolute left-0 top-0 z-0 h-full w-full">
        <div className="absolute left-1/2 top-1/2 h-[1px] w-[1px]">
          ...
          <div
            className="radar-pulse absolute h-[450px] w-[450px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-accent/10"
      ...
      <div className="relative z-10 flex w-full flex-col items-center gap-6 px-12 text-center">
        {files.length > 0 || dragging ? (
          <div className="flex w-full flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center justify-between px-2">
              ...
              {!dragging && (
                <div className="flex gap-2">
                  <button
                    className="flex h-9 w-9 items-center justify-center rounded-full bg-bg-elevated hover:bg-bg-surface transition-colors border border-border/40"
                    onClick={onPickFiles}
                    title="Add Files"
                  >
            ...
            <div
              className={`flex flex-col gap-2 max-h-[320px] min-h-[100px] overflow-y-auto p-1 scrollbar-none rounded-2xl transition-all ${dragging ? "border-2 border-dashed border-accent/40 bg-accent/5" : ""}`}
            >
              {files.length > 0
                ? files.map((f) => (
                    <div
                      key={f.id}
                      className="group flex items-center gap-4 rounded-2xl border border-border/40 bg-bg-surface/60 p-3 backdrop-blur-md transition-all hover:border-accent/40 hover:bg-bg-elevated/80"
                    >
          ...
          <>
            <div
              className="group relative h-32 w-32 cursor-pointer transition-transform hover:scale-110"
              onClick={onPickFiles}
            >
            ...
            >
              <div
                className={`grid h-16 w-16 place-items-center rounded-2xl border-2 transition-all ${
                  active
                    ? "border-accent bg-accent/15 shadow-cute"
                    : "border-border/40 bg-bg-surface backdrop-blur-sm"
                }`}
              >
