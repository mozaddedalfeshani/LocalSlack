    ...
    <div className="space-y-12">
      ...
      <div className="flex items-start justify-between gap-8">
        <div className="space-y-1">
          ...
          <p className="text-xs text-text-muted">
            The internal port LocalSlack uses to communicate.
          </p>
        </div>
      ...
      <div className="flex items-start justify-between gap-8">
        <div className="space-y-1">
          ...
          <p className="text-xs text-text-muted">
            Only these IP addresses can send you files.
          </p>
        </div>
      ...
      <div className="flex items-start justify-between gap-8">
        <div className="space-y-1">
          ...
          <p className="text-xs text-text-muted">
            Prevent specific addresses from connecting.
          </p>
        </div>
        <div className="flex-1 max-w-sm">
          <textarea
      ...
      <div className="space-y-6 pt-4 border-t border-border/10">
        <label className="flex items-center justify-between cursor-pointer group">
          <div className="space-y-1">
            ...
            <p className="text-xs text-text-muted">
              Hide your device from network discovery scans.
            </p>
          </div>
          <div
            className={`relative h-6 w-11 rounded-full transition-colors ${settings.hidden ? "bg-accent" : "bg-border/60"}`}
          >
            <div
              className={`absolute left-1 top-1 h-4 w-4 rounded-full bg-white transition-transform ${settings.hidden ? "translate-x-5" : "translate-x-0"}`}
            />
            <input
              type="checkbox"
              className="hidden"
      ...
      <div className="rounded-2xl bg-bg-elevated/20 p-6 border border-border/10">
        ...
        <div className="flex flex-wrap gap-2">
          {ips.map((ip) => (
            <div
              key={ip}
              className="rounded-lg bg-bg-surface px-3 py-1.5 text-xs font-medium text-text-secondary border border-border/40"
            >
              {ip}
            </div>
          ))}
          {ips.length === 0 && (
            <p className="text-xs text-text-muted italic">
              Identifying local network address...
            </p>
          )}
        </div>
      </div>
