    ...
    <div className="space-y-12">
      ...
      <div className="flex items-start justify-between gap-8">
        <div className="space-y-1">
          ...
          <p className="text-xs text-text-muted">
            Keep the cute LocalSlack light look everywhere.
          </p>
        </div>
      ...
      <div className="flex items-start justify-between gap-8">
        <div className="space-y-1">
          ...
          <p className="text-xs text-text-muted">
            Adjust the size of the interface text.
          </p>
        </div>
        <div className="flex-1 max-w-sm">
          <select
            ...
            onChange={(e) =>
              onChange({ fontSize: e.target.value as AppSettings["fontSize"] })
            }
          >
      ...
      <div className="flex items-start justify-between gap-8">
        <div className="space-y-1">
          ...
          <p className="text-xs text-text-muted">
            Personalize the primary color theme.
          </p>
        </div>
      ...
      <div className="space-y-6 pt-4 border-t border-border/10">
        <label className="flex items-center justify-between cursor-pointer group">
          <div className="space-y-1">
            ...
            <p className="text-xs text-text-muted">
              Reduce spacing to show more content at once.
            </p>
          </div>
          <div
            className={`relative h-6 w-11 rounded-full transition-colors ${settings.compactMode ? "bg-accent" : "bg-border/60"}`}
          >
            <div
              className={`absolute left-1 top-1 h-4 w-4 rounded-full bg-white transition-transform ${settings.compactMode ? "translate-x-5" : "translate-x-0"}`}
            />
            <input
              type="checkbox"
              className="hidden"
