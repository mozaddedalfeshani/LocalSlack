...
export function SettingsMenu({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <aside className="fixed inset-y-0 right-0 z-40 flex w-[420px] max-w-full flex-col border-l border-border bg-bg-secondary shadow-panel">
      ...
      <div className="flex gap-2 border-b border-border p-3">{(["general", "network", "appearance"] as const).map((item) => <button key={item} className={`secondary-button flex-1 ${tab === item ? "selected" : ""}`} onClick={() => setTab(item)}>{t(`settings.${item}`)}</button>)}</div>
      ...
      <footer className="border-t border-border p-4"><button className="primary-button w-full" onClick={() => save(settings)}>{t("settings.save")}</button></footer>
    </aside>
  );
}
