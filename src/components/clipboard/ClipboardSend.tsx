    ...
    <section className="space-y-4">
      ...
      <div className="flex justify-between gap-3">
        <button
          ...
          onClick={async () => setText(await clipboard.read())}
        >
        ...
        <button
          ...
          disabled={!selectedDevice || !text.trim()}
          onClick={() => selectedDevice && clipboard.send(selectedDevice, text)}
        >
          <Send size={16} /> {t("clipboard.send")}
        </button>
      </div>
      {clipboard.error && (
        <p className="text-sm text-error">{clipboard.error}</p>
      )}
    </section>
  );
}
