...
export function ReceiveDialog({
  ...
  return (
    <div className="fixed inset-0 z-[100] grid place-items-center bg-bg-primary/80 px-4 backdrop-blur-sm">
      ...
      >
        <Monitor
          className="mx-auto mb-4 h-12 w-12 text-text-primary"
          aria-hidden="true"
        />
        <h2 className="break-words text-3xl font-semibold text-text-primary">
          {sender.name}
        </h2>
        <p className="mt-3 text-base text-text-secondary">
          wants to send you {files.length === 1 ? "a file" : "files"}
        </p>
        ...
        <div className="flex justify-center gap-3">
          <button
            className="secondary-button bg-error/20 text-text-primary hover:border-error"
            onClick={onReject}
          >
            <X size={18} />
            {t("transfer.reject")}
