...
export function ReceivingFilesDialog({
  ...
  const items = transfer.files.map((file) => {
    const current = progress.find((item) => item.sessionId === transfer.sessionId && item.fileId === file.id);
  ...
  const receivedBytes = items.reduce((sum, item) => sum + Math.min(item.received, item.total), 0);
  const overallPercent = totalBytes ? Math.min(100, Math.round((receivedBytes / totalBytes) * 100)) : 0;
    ...
    <aside className="fixed inset-x-4 bottom-4 z-40 mx-auto max-w-3xl rounded-md border border-border bg-bg-secondary p-4 shadow-panel">
      <div className="mb-3 flex items-start justify-between gap-4">
        <div className="min-w-0">
          <h2 className="text-lg font-semibold text-text-primary">{done ? "Finished" : "Receiving files"}</h2>
        ...
        {items.map(({ file, current, total, percent }) => (
          <div key={file.id} className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs text-text-muted">{done || percent >= 100 ? "Done" : current ? formatSpeed(current.speedBps) : "Waiting..."}</p>
